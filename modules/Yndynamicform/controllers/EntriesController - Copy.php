            ...
            if(count($fieldMapArray) > 0 ) {
              ...
              if($item_id) {
                ...
                $this->view->ajaxform_field_id = $profileTypeField->field_id;
                  ...
                  // DEEPAK
                  $tempPostVal = $_POST;
                  unset($tempPostVal['save_form']);
                  unset($tempPostVal['arrayIsValid']);
                  $val = $tempPostVal; // (array)$new_entry_form->getValues();
...
//                $val = (array)$new_entry_form->getValues();
                ...
                foreach ($val as $key=>$value){
                    ...
                    $valueSaved = $_POST[$key]; // $new_entry_form->getValue($key);
                    if( !isset($valueSaved) )
                        continue;
...
//                        }
          ...
          //  }
            ...
            // set min & max label for number fieldtempPostVal['save_form']);
                  if(isset($tempPostVal['arrayIsValid'])) unset($tempPostVal['arrayIsValid']);
                  $val = $tempPostVal; // (array)$new_entry_form
            
                  $tempPostVal = $_POST;
                  if(isset($tempPostVal['save_form'])) unset($tempPostVal['save_form']);
                  if(isset($tempPostVal['arrayIsValid'])) unset($tempPostVal['arrayIsValid']);
                  $val = $tempPostVal; // (array)$edit_entry_form->getValues();
...
//            if($this->getRequest()->isPost() && $edit_entry_form->isValid($this->getRequest()->getPost())) {
            if($this->getRequest()->isPost()) {
                ...
                foreach ($val as $key=>$value){
                    ...
                    $valueSaved = $_POST[$key]; // $edit_entry_form->getValue($key);
                    if( !isset($valueSaved) )
                        continue;
